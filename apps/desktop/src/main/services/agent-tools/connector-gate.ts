...
export const CONNECTOR_OUTCOME_TEXT: Record<
  ...
  (label: string, detail?: string) => string
> = {
  // Two things the model does not otherwise learn: the just-registered tool
  // names (it guesses otherwise), and that the attached account is "me".
  ...
  async respond(request: RespondConnectorRequest): Promise<void> {
    ...
    const detail =
      request.outcome === "connected"
        ...
        : request.error;
    waiting.resolve(
      CONNECTOR_OUTCOME_TEXT[request.outcome](waiting.label, detail)
    );
    this.emitEvent({
