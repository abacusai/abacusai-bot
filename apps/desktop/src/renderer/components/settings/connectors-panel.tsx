...
export const ConnectorsPanel = (): JSX.Element => {
  ...
  // An agent-key card stores its credential and installs nothing; agents
  // spawned from now on carry it in their environment, running ones do not.
  const saveAgentKey = useCallback(
    async (connector: AgentKeyConnector, value: string) => {
      ...
      try {
        ...
        else toast.success(t("connectors.removed", { name: connector.name }));
        await queryClient.invalidateQueries({
          queryKey: settingsQueryKeys.connectors.storedKeys,
        });
      } catch (error) {
